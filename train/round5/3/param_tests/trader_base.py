...
import jsonpickle, statistics, math, json
...
# —— 运行时参数，可在 traderData 里覆写（见下方 PARAM_DEFAULT）
PARAM_DEFAULT = {
    # ▸ Passive MM
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # First run - initialize with our test parameters
        if not state.traderData:
            # Load parameters from uploaded_params.json if it exists
            try:
                with open('uploaded_params.json', 'r') as f:
                    custom_params = json.load(f)
                    params.update(custom_params)
            except:
                pass  # Use default parameters if file doesn't exist
                
            traderData = jsonpickle.encode({"params": params, "persist": self.state})
        else:
