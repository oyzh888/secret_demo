...
# —— 运行时参数 - Y-takeBest: 直接hit/lift最优价格
PARAM_DEFAULT = {
    # ▸ Passive MM
    "mm_spread": 0,              # 做市基础半边价差（ticks）- 设为0让价格更激进
    "mm_size_frac": 0.20,        # 单边挂单数量 = limit × mm_size_frac
    "mm_vol_k": 0.0,             # 波动 × k → 额外外扩价差 - 设为0让价格更激进
    # ▸ Panic 出清
    "panic_ratio": 0.80,
    ...
    "counterparties": {          # 每个对手的默认阈值 & 下单比例
        "Camilla": {"thr": 2, "size_frac": 0.10},
        "Caesar":  {"thr": 2, "size_frac": 0.10},
        "ANON":    {"thr": 2, "size_frac": 0.10}  # 添加匿名交易者
    },
...
class Trader:
    ...
    def _update_streaks(self, market_trades, params):
        for prod, trades in market_trades.items():
            for t in trades:
                # 补丁2：没有姓名也计 streak
                cp = (t.buyer if t.buyer not in (None, "SUBMISSION", "") else
                      t.seller if t.seller not in (None, "SUBMISSION", "") else None)
                # 如果仍为空，就用占位 "ANON"
                if cp is None or cp == "":
                    cp = "ANON"
    ...
    def _passive_mm(self, p, depth, pos, params, orders):
        ...
        if pos < LIMIT[p]:
            # 直接以最优卖价买入
            orders.append(Order(p, best_ask, min(size, LIMIT[p] - pos)))
        if pos > -LIMIT[p]:
            # 直接以最优买价卖出
            orders.append(Order(p, best_bid, -min(size, LIMIT[p] + pos)))
