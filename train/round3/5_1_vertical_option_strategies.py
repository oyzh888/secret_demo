...
class Trader:
    def __init__(self):
        ...
        self.positions = {}  # Format: {spread_key: {"lower": {"symbol": str, "price": float, "quantity": int}, "upper": {...}}}
        self.take_profit_threshold = 0.3  # 30% profit threshold
        self.max_profit_threshold = 0.5  # 50% max profit threshold
    ...
    def get_spread_key(self, lower_strike: int, upper_strike: int) -> str:
        return f"{lower_strike}_{upper_strike}"

    def calculate_current_profit(self, spread_key: str, state: TradingState) -> float:
        """Calculate current profit for a spread position"""
        if spread_key not in self.positions:
            return 0.0
            
        position = self.positions[spread_key]
        lower = position["lower"]
        upper = position["upper"]
        
        if lower["symbol"] not in state.order_depths or upper["symbol"] not in state.order_depths:
            return 0.0
            
        # Get current market prices
        lower_bid = max(state.order_depths[lower["symbol"]].buy_orders.keys())
        upper_ask = min(state.order_depths[upper["symbol"]].sell_orders.keys())
        
        # Calculate profit
        lower_profit = (lower_bid - lower["price"]) * lower["quantity"]
        upper_profit = (upper["price"] - upper_ask) * upper["quantity"]
        
        return lower_profit + upper_profit

    def should_take_profit(self, spread_key: str, state: TradingState) -> bool:
        """Check if we should take profit on a spread position"""
        if spread_key not in self.positions:
            return False
            
        current_profit = self.calculate_current_profit(spread_key, state)
        entry_cost = (self.positions[spread_key]["lower"]["price"] - 
                     self.positions[spread_key]["upper"]["price"]) * self.positions[spread_key]["lower"]["quantity"]
        
        profit_percentage = current_profit / abs(entry_cost)
        
        # Take profit if we've reached our threshold
        if profit_percentage >= self.take_profit_threshold:
            # If profit is very high, take profit more aggressively
            if profit_percentage >= self.max_profit_threshold:
                return True
            # Otherwise, take profit with some probability based on profit level
            return np.random.random() < (profit_percentage - self.take_profit_threshold) / (self.max_profit_threshold - self.take_profit_threshold)
    ...
    def run(self, state: TradingState):
        ...
        # First check for take-profit opportunities
        for spread_key in list(self.positions.keys()):
            if self.should_take_profit(spread_key, state):
                position = self.positions[spread_key]
                lower = position["lower"]
                upper = position["upper"]
                
                # Close the spread
                if lower["symbol"] not in result:
                    result[lower["symbol"]] = []
                if upper["symbol"] not in result:
                    result[upper["symbol"]] = []
                    
                # Sell lower strike, buy upper strike
                result[lower["symbol"]].append(Order(lower["symbol"], max(state.order_depths[lower["symbol"]].buy_orders.keys()), -lower["quantity"]))
                result[upper["symbol"]].append(Order(upper["symbol"], min(state.order_depths[upper["symbol"]].sell_orders.keys()), upper["quantity"]))
                
                print(f"TAKING PROFIT: Closing spread {spread_key}")
                del self.positions[spread_key]
        ...
        for i in range(len(strikes) - 1):
            ...
            spread_key = self.get_spread_key(lower_strike, upper_strike)
            
            # Skip if we already have this spread
            if spread_key in self.positions:
                continue
            ...
            if bull_profit > 0 and position_limit > 0:
                ...
                lower_price = min(state.order_depths[lower_voucher].sell_orders.keys())
                upper_price = max(state.order_depths[upper_voucher].buy_orders.keys())
                
                result[lower_voucher].append(Order(lower_voucher, lower_price, position_limit))
                result[upper_voucher].append(Order(upper_voucher, upper_price, -position_limit))
                
                # Record the position
                self.positions[spread_key] = {
                    "lower": {"symbol": lower_voucher, "price": lower_price, "quantity": position_limit},
                    "upper": {"symbol": upper_voucher, "price": upper_price, "quantity": position_limit}
                }
                ...
                # Sell lower strike, buy upper strike
                lower_price = max(state.order_depths[lower_voucher].buy_orders.keys())
                upper_price = min(state.order_depths[upper_voucher].sell_orders.keys())
