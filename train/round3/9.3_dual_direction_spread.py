...
class Trader:
    ...
    def calculate_bull_spread_profit(self, lower_strike: int, upper_strike: int, state: TradingState) -> float:
        """Calculate bull spread profit potential"""
        ...
        spread_cost = lower_ask - upper_bid
        spread_profit = upper_strike - lower_strike - spread_cost
        
        return spread_profit

    def calculate_bear_spread_profit(self, lower_strike: int, upper_strike: int, state: TradingState) -> float:
        """Calculate bear spread profit potential"""
        lower_voucher = f"VOLCANIC_ROCK_VOUCHER_{lower_strike}"
        upper_voucher = f"VOLCANIC_ROCK_VOUCHER_{upper_strike}"
        ...
        spread_cost = upper_ask - lower_bid
        spread_profit = upper_strike - lower_strike - spread_cost
    ...
    def run(self, state: TradingState):
        ...
        # Evaluate both bull and bear spreads
        strikes = [9500, 9750, 10000, 10250, 10500]
        for i in range(len(strikes) - 1):
            lower_strike = strikes[i]
            upper_strike = strikes[i + 1]
            
            # Calculate both bull and bear spread profits
            bull_profit = self.calculate_bull_spread_profit(lower_strike, upper_strike, state)
            bear_profit = self.calculate_bear_spread_profit(lower_strike, upper_strike, state)
            ...
            position_limit = min(
                self.position_limits[lower_voucher] - abs(current_positions.get(lower_voucher, 0)),
                self.position_limits[upper_voucher] - abs(current_positions.get(upper_voucher, 0))
            )
            ...
            # Execute the more profitable spread
            if bull_profit > bear_profit and bull_profit > 0 and max_position > 0:
                ...
                print(f"DUAL SPREAD: Bull Spread - Buy {lower_voucher}, Sell {upper_voucher}, Profit: {bull_profit:.2f}, Size: {max_position}")
                ...
                print(f"DUAL SPREAD: Bear Spread - Sell {lower_voucher}, Buy {upper_voucher}, Profit: {bear_profit:.2f}, Size: {max_position}")
