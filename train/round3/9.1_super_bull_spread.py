...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # Evaluate bull spreads aggressively
        strikes = [9500, 9750, 10000, 10250, 10500]
        for i in range(len(strikes) - 1):
            lower_strike = strikes[i]
            upper_strike = strikes[i + 1]
            
            spread_profit = self.calculate_bull_spread_profit(lower_strike, upper_strike, state)
            ...
            if spread_profit > 0 and max_position > 0:
                ...
                print(f"SUPER BULL SPREAD: Buy {lower_voucher}, Sell {upper_voucher}, Profit: {spread_profit:.2f}, Size: {max_position}")
